    ...
    if (dat.data == undefined) {
        ...
        return (
            <svg width={width} height={height}>
                <text className={st.title} x={innerWidth / 2} y={30}>
                    {props.address[0]} {props.address[1]}, {props.suburb}
                </text>
                <g transform={`translate(${margin.left},${margin.top})`}>
                    ...
                    {yScale.domain().map((tickValue, index) => (
                        ...
                        >
                            <text x={-50} style={{ textAnchor: "middle" }}>
                                {tickValue}
                            </text>
                        </g>
                    ))}
                    <text
                        className={st.axisLabelX}
                        x={innerWidth / 2}
